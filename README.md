...
- **Panel open/close & visibility** — `node(ref)` reads `active`/`activeInHierarchy`/
  ...
  after an action (e.g. press buyfeature → its block opens).
...
To check a panel opened ("press buyfeature → its block appears"): `snapshot({includeInactive:true})`
...
see the game *react* (panels open, reels spin, numbers change) — not a moving cursor or a
