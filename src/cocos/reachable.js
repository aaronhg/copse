 ...
 * needs from `cc` (with the same tree-shake-safe class-NAME-string fallback runtime.js uses), so
 * it shares nothing with the base runtime. Cross-Cocos-version strategy: PROBE capabilities at
 * runtime, never branch on a version string (protected/forked builds lie); degrade down a ladder
 * to a public-API floor and fail LOUD ('unsure') instead of guessing.
 * @param {any} cc
...
export function makeReachable(cc) {
  ...
  //     by a non-consumer) — fail LOUD, never a confident pass.
